	...
	perform_test : function() {
		new Test.Unit.Runner({
			...
			setup: function() {
				...
				this.square_brackets = "<ul>[% this.animals.each(function(animal){%]<li>[%= animal %]</li>[%});%]</ul>";
				this.square_brackets_no_this = "<ul>[% animals.each(function(animal){%]<li>[%= animal %]</li>[%});%]</ul>";
				this.angle_brackets_no_this	 = "<ul><% animals.each(function(animal){%><li><%= animal %></li><%});%></ul>";
			},
			...
			test_update : function() { with(this) {
				...
				assertEqual("<ul><li>sloth</li><li>bear</li><li>monkey</li></ul>", 
					document.getElementById('update_me').innerHTML.toLowerCase().replace(/\r\n/g, '') 
				);
			}},
			...
			test_render_with_double: function() { with(this) {
				...
				assertEqual("<% replace_me %><ul><li>sloth</li><li>bear</li><li>monkey</li></ul>", 
					compiled
				);
			}},
			...
			test_error_forgot_opening_carrot: function() { with(this) {
				var text = "<ul><% animals.each(function(animal){%><li> animal %></li><%});%></ul>";
				var compiled = new EJS({text: text}).render({animals: this.animals});
				assertEqual("<ul><li> animal %></li><li> animal %></li><li> animal %></li></ul>", 
					compiled
				);
			}},
